...
#to connect my agent to db


#to find the product rating (avg rating of the search product)- for the search product
def get_product_rating(product_id: int) -> dict:
    """Return average rating and review count for a single product."""
    conn = sqlite3.connect(DB_PATH) #to connct to db
    cursor = conn.cursor()
    cursor.execute(
...
#to find the list of product rating (avg rating of the search product)
